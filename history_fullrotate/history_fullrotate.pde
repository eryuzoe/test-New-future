...
boolean hasCompletedCircle = false; // 标记是否完成了一整圈
...
void checkCompleteCircle() {
  float angleDiff = angle - lastButtonAngle;
  if (angleDiff < -PI) {
    angleDiff += TWO_PI;
  ...
  if (abs(totalRotation) >= TWO_PI) {
    ...
    totalRotation = 0; // 重置总旋转角度
    hasCompletedCircle = true;
  }

  lastButtonAngle = angle;
}
...
void mouseReleased() {
  if (hasCompletedCircle) {
    sectorAngle = random(100 * (PI / 180), 150 * (PI / 180)); // 改变扇形的角度
