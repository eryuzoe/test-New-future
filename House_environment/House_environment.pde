...
void drawOuterCircleGradient(int cx, int cy, int radius) {
  ...
  int blue = color(0, 0, 255); // 蓝色
  int orange = color(255, 165, 0); // 橙色
  ...
  for (int i = 0; i < segments; i++) {
    ...
    if (inter < 0.25) { // 从白色渐变到蓝色
      c = lerpColor(white, blue, inter * 4);
    } else if (inter < 0.5) { // 从蓝色渐变到白色
      c = lerpColor(blue, white, (inter - 0.25) * 4);
    } else if (inter < 0.75) { // 从白色渐变到橙色
      c = lerpColor(white, orange, (inter - 0.5) * 4);
    } else { // 从橙色渐变到白色
      c = lerpColor(orange, white, (inter - 0.75) * 4);
    }
...
void updateBackgroundColor() {
  ...
  if (inter < 0.25) {
    bgColor = lerpColor(white, blue, inter * 4);
  } else if (inter < 0.5) {
    bgColor = lerpColor(blue, white, (inter - 0.25) * 4);
  } else if (inter < 0.75) {
    bgColor = lerpColor(white, orange, (inter - 0.5) * 4);
  } else {
    bgColor = lerpColor(orange, white, (inter - 0.75) * 4);
  }
